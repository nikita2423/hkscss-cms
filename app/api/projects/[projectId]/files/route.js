...
// const ALLOW_ORIGINS = new Set(['http://localhost:3000']); // caller app
...
//   // Echo what browser asked to send (prevents "Request header not allowed" errors)
//   const reqHeaders = req?.headers.get('access-control-request-headers');
//   const reqMethod = req?.headers.get('access-control-request-method');
//   h.set('Access-Control-Allow-Headers', reqHeaders || 'Content-Type, Authorization');
//   h.set('Access-Control-Allow-Methods', reqMethod || 'GET,POST,PUT,PATCH,DELETE,OPTIONS');
//   h.set('Access-Control-Max-Age', '86400');
//   // If you need cookies/Authorization across origins:
//   // h.set('Access-Control-Allow-Credentials', 'true');
//   return h;
// }

// function applyCORS(res, origin, req) {
//   const h = corsHeaders(origin, req);
...
//     if (!projectId) {
//       return applyCORS(
//         NextResponse.json({ error: 'The project ID cannot be empty' }, { status: 400 }),
//         pickOrigin(request)
//       );
//     }
...
//     const getAllIds = (searchParams.get('getAllIds') || '').toLowerCase() === 'true';

//     if (getAllIds) {
//       const allFiles = await getUploadFilesPagination(projectId, 1, 9999, fileName);
//       const allFileIds = allFiles.data?.map(f => String(f.id)) || [];
//       return applyCORS(NextResponse.json({ allFileIds }), pickOrigin(request));
//     }

//     const files = await getUploadFilesPagination(projectId, page, pageSize, fileName);
//     return applyCORS(NextResponse.json(files), pickOrigin(request));
//   } catch (error) {
//     console.error('Error obtaining file list:', String(error));
//     return applyCORS(
//       NextResponse.json({ error: error?.message || 'Error obtaining file list' }, { status: 500 }),
//       pickOrigin(request)
//     );
//   }
// }
