...
//       // Persistent storage (Blob/S3/etc.)
//       const buf = await fetchBuffer(file.path);
//       if (fileName.endsWith('.pdf')) {
//         mdText = await pdf2md(buf); // convert PDF -> markdown
//       } else {
//         mdText = buf.toString('utf8'); // .md
//       }
//     } else {
//       const projectRoot = process.env.VERCEL ? '/tmp' : await (await import('@/lib/db/base')).getProjectRoot();
//       const projectPath = path.join(projectRoot, projectId);
//       let filePath = path.join(projectPath, 'files', fileName);

//       // If not .md, try the .md sibling produced by your PDF pipeline
...
//       } catch {
//         throw new Error(`文件 ${fileName} 不存在或不可用（在函数运行时的文件系统中找不到）。`);
//       }

//       mdText = await fs.readFile(filePath, 'utf8');
//     }
...
export async function splitProjectFile(
  projectId: string,
  file: { id?: string, fileId?: string, fileName: string, path?: string }
) {
  const fileId = file.fileId ?? file.id;
  const { fileName } = file;

  try {
    // 1) LOAD CONTENT (prefer persistent URL; fallback to disk in local dev)
    let mdText: string;

    if (file.path && /^https?:\/\//i.test(file.path)) {
      const buf = await fetchBuffer(file.path);
      mdText = fileName.endsWith('.pdf') ? await pdf2md(buf) : buf.toString('utf8');
    } else {
      // local fallback only
      const baseRoot = process.env.VERCEL ? '/tmp' : await getProjectRoot();
      const projectPathLocal = path.join(baseRoot, projectId);
      let localPath = path.join(projectPathLocal, 'files', fileName);
      if (!localPath.endsWith('.md')) {
        localPath = path.join(projectPathLocal, 'files', fileName.replace(/\.[^/.]+$/, '.md'));
      }
      await fs.access(localPath); // throws if missing
      mdText = await fs.readFile(localPath, 'utf8');
    }

    if (!mdText?.trim()) throw new Error('Empty content after loading—cannot split.');

    // 2) SPLIT INTO CHUNKS using the in-memory text (no disk read again)
    const baseRoot = process.env.VERCEL ? '/tmp' : await getProjectRoot();
    const projectPath = path.join(baseRoot, projectId);

    const chunksData = await splitFileByType({
      projectPath,
      fileContent: mdText,
      fileName,
      projectId,
    ...
    // Use createMany inside saveChunks for speed on serverless
    await saveChunks(chunksData);

    // 3) TOC from the same mdText
    const tocJSON = markdownSplitter.extractTableOfContents(mdText);
    const toc = markdownSplitter.tocToMarkdown(tocJSON, { isNested: true });

    // 4) Persist TOC file only in local dev (Vercel /tmp is ephemeral)
    if (!process.env.VERCEL) {
      const tocDir = path.join(projectPath, 'toc');
      await ensureDir(tocDir);
      const tocPath = path.join(tocDir, `${path.basename(fileName, path.extname(fileName))}-toc.json`);
      await fs.writeFile(tocPath, JSON.stringify(tocJSON, null, 2));
    }
    // (If you need TOC persisted on Vercel, upload JSON to Blob/S3 here.)
