...
export async function processAnswerGenerationTask(task) {
  try {
    ...
    const questionsWithoutAnswers = await db.questions.findMany({
      where: {
        projectId,
    ...
    const processQuestion = async question => {
      try {
        ...
        const latestTask = await db.task.findUnique({ where: { id: task.id } });
        ...
        // 更新任务进度
        const progressNote = `已处理: ${
          successCount + errorCount
        }/${totalCount}, 成功: ${successCount}, 失败: ${errorCount}, 共生成数据集: ${totalDatasets}`;
        await updateTask(task.id, {
